...
def check_plagiarism():
    ...
    # Split text into clear sentences
    ...
    with DDGS() as ddgs:
        for sentence in sentences[:15]:
            ...
            # Ignore very short common phrases to avoid false reds
            if len(sentence.split()) < 6:
                analysis.append({"sentence": sentence, "plagiarized": False})
                continue

            is_plagiarized = False
            query = f'"{sentence}"' # Forced Exact Match

            # Layered search across 4 engines
            try:
            ...
            analysis.append({"sentence": sentence, "plagiarized": is_plagiarized})
